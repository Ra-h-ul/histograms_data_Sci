...
over<-ovun.sample(Exited~.,data=new.knn.imputed , method = 'over' , N=15926)$data
under<-ovun.sample(Exited~.,data=new.knn.imputed , method = 'under' , N=4074)$data
both<-ovun.sample(Exited~.,data=new.knn.imputed , method = 'both' , 
                  p=0.5,  seed=123,  N=7963)$data
...
#over
trainm.over<- sparse.model.matrix(Exited~. , -11 , data=train.over)
train_label.over <-train.over[,"Exited"]
train_matrix.over<-xgb.DMatrix( data = as.matrix(trainm.over), label=train_label.over)


testm.over <- sparse.model.matrix(Exited~. , -11, data=test.over)
test_label.over<-test.over[,"Exited"]
test_matrix.over<-xgb.DMatrix(data=as.matrix(testm.over)  , label = test_label.over)

#under
trainm.under<- sparse.model.matrix(Exited~. , -11 , data=train.under)
train_label.under <-train.under[,"Exited"]
train_matrix.under<-xgb.DMatrix( data = as.matrix(trainm.under), label=train_label.under)


testm.under <- sparse.model.matrix(Exited~. , -11, data=test.under)
test_label.under<-test.under[,"Exited"]
test_matrix.under<-xgb.DMatrix(data=as.matrix(testm.under)  , label = test_label.under)

#both
trainm.both<- sparse.model.matrix(Exited~. , -11 , data=train.both)
train_label.both <-train.both[,"Exited"]
train_matrix.both<-xgb.DMatrix( data = as.matrix(trainm.both), label=train_label.both)


testm.both <- sparse.model.matrix(Exited~. , -11, data=test.both)
test_label.both<-test.both[,"Exited"]
